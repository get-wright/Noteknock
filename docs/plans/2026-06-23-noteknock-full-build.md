...
- **Screens** (10): Login, Register (new, mirrors login split-screen), Dashboard, Empty, Study, Editor, Quiz, QuizResult, Review, Profile, Streak. Port each `.dc.html` `isX` block.
...
**Tag extraction:** same plain-text walk → regex `(?:(?<=^#)|(?<=\s#))[a-zA-Z0-9_-]+(?=\s|$)` → lowercase + dedupe → rewrite `note_tags` rows.
...
| POST | `/api/attachments` | yes | multipart | `{url}` | 4 |
...
**Step 1:** Create `docker-compose.yml` with three services: `postgres:16` (volume, healthcheck), `backend` (build `./backend`, env from `.env`, port 8000, depends_on postgres healthy), `frontend` (build `./backend`, Vite dev on 5173). Postgres uses `POSTGRES_USER=noteknock`, `POSTGRES_PASSWORD=noteknock`, `POSTGRES_DB=noteknock`.
...
**Step 1: Write failing tests** using `httpx.AsyncClient` + `ASGITransport` against the FastAPI app:
```python
@pytest.mark.asyncio
async def test_health(): ...
@pytest.mark.asyncio
async def test_register_then_token_then_authcheck(): ...  # full flow
@pytest.mark.asyncio
async def test_register_duplicate_email_409(): ...
@pytest.mark.asyncio
async def test_token_bad_creds_401(): ...
@pytest.mark.asyncio
async def test_auth_check_no_token_401(): ...
```
Tests use a separate test DB or transaction rollback fixture. Register happy path: POST `/api/register` → 200 `Token`; POST `/api/token` with form → 200 `Token`; GET `/api/auth-check` with Bearer → 200 `"OK"`.
...
**Step 1:** `client.ts` — `fetch` wrapper: attaches `Authorization: Bearer <token>` from `localStorage`, parses JSON, throws on non-2xx with the server's error string. `auth.ts` — `register({name,email,password})`, `login(email,password)`, `authCheck()`.

**Step 2:** `AuthContext` — stores token in `localStorage`, exposes `{user, token, login, register, logout}`, on mount calls `authCheck` to validate. `ProtectedRoute` — redirects to `/login` if no valid token.
...
```python
BLOCKS = [
  {"type":"paragraph","content":[{"type":"text","text":"Tích phân #toan từng phần","styles":{}}]},
  {"type":"code","content":[{"type":"text","text:"#notatag inside code"}]},
  {"type":"paragraph","content":[{"type":"text","text":"ILATE #ly helps","styles":{}}]},
]
def test_blocks_to_text_skips_code():
    assert blocks_to_text(BLOCKS) == "Tích phân #toan từng phần ILATE #ly helps"
def test_extract_tags():
    assert extract_tags(BLOCKS) == ["toan", "ly"]
...
**Step 3: Write `tags.py`** `extract_tags(blocks: list) -> list[str]` — `blocks_to_text` → regex `(?:(?<=^#)|(?<=\s#))[a-zA-Z0-9_-]+(?=\s|$)` → lowercase + dedupe (preserve order).
...
- `term=#toan` rewrites to tag filter, returns only notes with that tag
...
- quoted `"phrase"` → phrase tsquery, no tags
- operators `title:`/`content:`/`tags:`, `AND`/`OR`/`NOT` → translate to tsquery boolean ops
...
**Step 2: `schema.ts`** — define a custom `formula` block via `createReactBlockSpec` matching the prototype's formula style (mono font, `var(--accent)` color, `var(--accent-tint)` bg, 12px radius, `contenteditable=false`, `content: "none"`). Build `BlockNoteSchema.create({ blockSpecs: { ...defaultBlockSpecs, formula }, slashItems: [...defaults, formulaItem] })`.

**Step 3: `Editor.tsx`** — `useCreateBlockNote({ schema, initialContent })`, `useEditorChange` for autosave hook integration. `BlockNoteView` with `theme={{ light, dark }}` driven by current Peeky theme. Pass `data-ph="Bắt đầu viết bài học của bạn…"`.

**Step 4: `slashMenu.ts`** — `getDefaultReactSlashMenuItems(editor)` + custom `formula` item, filtered via `filterSuggestionItems`.
...
`Quiz` (id, note_id FK cascade unique, owner_id, created_at) + `QuizQuestion` (id, quiz_id FK cascade, position, prompt, options jsonb, correct_index, explanation). Migration + tests. Commit: `Add quiz models and migration`.
...
## Task P3.3: Frontend quiz screens (match mockup)

**Files:**
- Create: `frontend/src/pages/Quiz.tsx`, `frontend/src/pages/QuizResult.tsx`
- Create: `frontend/src/api/quiz.ts`

Port `isQuiz` (lines 349+) — question card with 4 option buttons (A/B/C/D badges), correct=coral fill, wrong=rose fill, "Tiếp" navigation, progress "1/10". On finish → `QuizResult` (port `isResult`): score ring, missed-questions accordion (collapsible, chevDown animation, "why" explanation). Wire to real quiz API. **Fix the quiz explanation dock spacing** noted in the spec (`dockStyle` margin collapses when explanation panel expands). Commit: `Add quiz and result screens wired to API`.

## Task P3.4: Review screen (match mockup)

**Files:**
- Create: `frontend/src/pages/Review.tsx`

Port `isReview` — due cards list (subject color dot + title + 3-bar strength indicator + "last reviewed" + chevRight), "Sắp tới" section. DB-backed: derive due notes from `updated_at` + a review schedule (spaced repetition — start with simple intervals). Commit: `Add review screen`.

## Task P3.5: Streak screen (match mockup)

**Files:**
- Create: `frontend/src/pages/Streak.tsx`

Port `isStreak` — big flame + "12 ngày" streak number, heatmap (port `heatmap` SVG builder verbatim from prototype — the calendar grid colored by activity), growth chart (port `growthChart` SVG), timeline of recent activity, ministats. DB-backed: compute streak from notes created/quiz activity per day. Commit: `Add streak screen with heatmap and growth chart`.

## Task P3.6: Profile screen (match mockup)
...
1. Open a note with content → "Bắt đầu Quiz" → generates quiz → answer → result screen with score + missed explanations.
2. Review screen shows due notes.
3. Streak screen shows heatmap populated by real activity.
4. Profile shows settings; theme toggle works.
...
`attachments` table, `POST /api/attachments` (multipart upload, Bearer auth) → PUT object in MinIO → insert row → return `{url}`. Editor image button → file picker → upload → insert image block with `url`. Commit: `Add attachment upload and storage`.
