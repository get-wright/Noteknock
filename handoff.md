...
- Latest completed plan task: expanded P4.2/P4.3 `attachments + editor upload previews` on branch `feature/note-attachments-preview` (`9182dc20`, `142d789c`, `52b13960`, `ac4cb5c8`, `b2702189`, `1bea91eb`, `9358fbb8`, `84015e5e`, `6df833b1`, `70f1b0ed`, `c372b3dc`, `f5a0d36e`, `0960a788`, `555cbe35`, `246cf5e7`, plus uncommitted DOCX/formula/PDF overlay/sidebar follow-up): added MinIO config, attachment metadata/migration, storage helpers, owner-scoped upload/preview/download/content API, frontend attachment client, BlockNote upload hook, custom PDF/material/DOCX preview blocks, editor upload feedback/slash-menu file picking, startup bucket bootstrap, public presigned URL endpoint config, editable formula blocks, a shared DOCX/PDF document-preview frame, and the design's collapsible desktop sidebar rail.
- Verification/review: all task-level spec and code-review gates passed after hardening follow-ups. Fresh verification after DOCX/formula/PDF overlay/sidebar follow-up: `pytest backend/tests/test_api_attachments.py -q` with `TEST_DATABASE_URL=postgresql+asyncpg://noteknock:noteknock@localhost:5433/noteknock_test` = 16 passed; `ruff check backend/app backend/tests` = passed; `npm --prefix frontend run build` = passed with existing chunk-size warning; local Docker backend/frontend rebuilt and `/health` returned `"OK"`; frontend container rebuilt after sidebar fix and is up on `http://localhost:5174`. `npm --prefix frontend run lint` is still blocked by the repo's missing ESLint 9 `eslint.config.*`.
- Blockers: none for the implemented checks. Manual browser smoke is still needed on the running stack: collapse/expand the desktop sidebar, verify BlockNote drag-menu placement, upload image/PDF/DOCX/material, confirm DOCX inline preview renders, verify the formula block can be edited/saved/reloaded, reload note, verify Study view previews, verify bucket creation, verify presigned URLs use `STORAGE_PUBLIC_ENDPOINT`, and verify cross-user preview/download/content isolation. Uploads omit `noteId` because note responses do not expose UUIDs yet, so attachments can remain owner-scoped orphans until a linking/GC task.
- Next planned task: run manual browser smoke on `http://localhost:5174`, then decide whether to add note UUID/linking + orphan cleanup or move to branch finishing/PR.
