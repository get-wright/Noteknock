...
- **Landing/login/register redesign** (2026-06-25): `Landing`, `Login`, and `Register` rebuilt with shared `AuthArtPanel`, `AuthFormShell`, `AuthInput`, `authMotion.ts`, and `frontend/src/styles/landing.css`; `motion` dependency for Peeky animations. Design/plan: `docs/plans/2026-06-25-landing-login-redesign-design.md`, `docs/plans/2026-06-25-landing-login-redesign-plan.md`. Frontend ESLint 9 flat config added (`frontend/eslint.config.js`). Verification: `npm run lint` and `npm run build` pass from `frontend/` (commit `60c4b1e5`).
...
- Latest completed plan task: expanded P4.2/P4.3 `attachments + editor upload previews` on branch `feature/note-attachments-preview` (`9182dc20`, `142d789c`, `52b13960`, `ac4cb5c8`, `b2702189`, `1bea91eb`, `9358fbb8`, `84015e5e`, `6df833b1`, `70f1b0ed`, `c372b3dc`, `f5a0d36e`, `0960a788`, `555cbe35`, `246cf5e7`, `5508d28f`): added MinIO config, attachment metadata/migration, storage helpers, owner-scoped upload/preview/download/content API, frontend attachment client, BlockNote upload hook, custom PDF/material/DOCX preview blocks, editor upload feedback/slash-menu file picking, startup bucket bootstrap, public presigned URL endpoint config, editable formula blocks, a shared DOCX/PDF document-preview frame, and the design's collapsible desktop sidebar rail.
- Verification/review: all task-level spec and code-review gates passed after hardening follow-ups. Fresh verification after merging latest `main` into the attachment branch: `pytest backend/tests/test_api_attachments.py -q` with `TEST_DATABASE_URL=postgresql+asyncpg://noteknock:noteknock@localhost:5433/noteknock_test` = 16 passed; `ruff check backend/app backend/tests` = passed; `npm --prefix frontend run lint` = passed with existing fast-refresh/hook warnings; `npm --prefix frontend run build` = passed with existing chunk-size warning. Earlier local Docker backend/frontend rebuilt and `/health` returned `"OK"`; frontend container rebuilt after sidebar fix and is up on `http://localhost:5174`. P4.1 verification also passed after OAuth fix.
- Blockers: none for the implemented checks. Important: the Google client secret was pasted in chat and should be rotated in Google Cloud Console if this transcript is stored/shared; `GOOGLE_REDIRECT_URI` must exactly match a Google Console authorized redirect URI and the SPA callback route. Manual browser smoke is still needed on the running stack: collapse/expand the desktop sidebar, verify BlockNote drag-menu placement, upload image/PDF/DOCX/material, confirm DOCX inline preview renders, verify the formula block can be edited/saved/reloaded, reload note, verify Study view previews, verify bucket creation, verify presigned URLs use `STORAGE_PUBLIC_ENDPOINT`, and verify cross-user preview/download/content isolation. Uploads omit `noteId` because note responses do not expose UUIDs yet, so attachments can remain owner-scoped orphans until a linking/GC task.
- Next planned task: run manual browser smoke on `http://localhost:5174`, rotate/verify Google OAuth credentials if needed, then decide whether to add note UUID/linking + orphan cleanup or continue branch finishing/deployment.
