...
- Latest completed plan task: expanded P4.2/P4.3 `attachments + editor upload previews` on branch `feature/note-attachments-preview` (`9182dc20`, `142d789c`, `52b13960`, `ac4cb5c8`, `b2702189`, `1bea91eb`, `9358fbb8`, `84015e5e`, `6df833b1`, `70f1b0ed`, `c372b3dc`, `f5a0d36e`, `0960a788`, `555cbe35`, `246cf5e7`): added MinIO config, attachment metadata/migration, storage helpers, owner-scoped upload/preview/download URL API, frontend attachment client, BlockNote upload hook, custom PDF/material preview blocks, editor upload feedback/slash-menu file picking, startup bucket bootstrap, and public presigned URL endpoint config.
- Verification/review: all task-level spec and code-review gates passed after hardening follow-ups. Fresh final verification after `246cf5e7`: `pytest tests/test_api_attachments.py tests/test_models_attachment.py tests/test_storage.py -q` = 27 passed; `pytest tests/test_services.py -q` = 13 passed; `ruff check app tests` = passed; `npm run build` from `frontend/` = passed with existing chunk-size warning. `docker compose config --quiet` could not run in the isolated worktree because `.env` is absent and compose requires it.
- Blockers: none for automated checks. Manual browser smoke is still needed with a real `.env`/running stack: upload image, PDF, DOCX/material, reload note, verify Study view previews, verify bucket creation, verify presigned URLs use `STORAGE_PUBLIC_ENDPOINT`, and verify cross-user preview/download URL isolation. DOCX inline rendering remains intentionally deferred; DOCX/Office files render as material cards. Uploads omit `noteId` because note responses do not expose UUIDs yet, so attachments can remain owner-scoped orphans until a linking/GC task.
- Next planned task: run manual full-stack smoke after creating `.env`, then decide whether to add note UUID/linking + orphan cleanup or move to branch finishing/PR.
